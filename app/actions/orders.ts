...
export async function getOrders(filters?: {
  ...
  if (filters?.search && filters.search.trim() !== "") {
    ...
    // 先搜尋 order_items 表中的商品名稱和規格，取得對應的 order_id
    ...
    // 建立搜尋條件：客戶名稱、批次、供應商
    let conditions = `customer_name.ilike.%${keyword}%,batch.ilike.%${keyword}%,supplier.ilike.%${keyword}%`

    // 如果 order_items 有找到匹配的訂單，加入 id.in 條件
    if (orderIdsFromItems.length > 0) {
      conditions += `,id.in.(${orderIdsFromItems.join(",")})`
    }
